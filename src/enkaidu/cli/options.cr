  ...
  class Options < SessionOptions
    ...
    def initialize(@renderer)
      @opts = OptionParser.parse do |parser|
        ...
        parser.on("-p NAME", "--provider=NAME",
          "The name of the provider: azure_openai, openai, or ollama") { |name| @provider_name = name }
        parser.on("-m NAME", "--model=NAME",
          "Some providers require a model.") { |name| @model_name = name }
        parser.on("-S", "--streaming", "Enable streaming mode") { @stream = true }
        parser.on("-R FILEPATH", "--recorder-file=FILEPATH",
